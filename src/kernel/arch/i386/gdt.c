...
#define GDT_ADDENTRY(m_entry, m_base, m_limit, m_access, m_flags)                                                     \
	...
	m_entry.base_low = (m_base & 0xffff), m_entry.base_mid = ((m_base >> 16) & 0xff), m_entry.access = m_access,      \
	m_entry.flags_limit = (((m_limit >> 16) & 0xf) | ((m_flags) & 0xf0)), m_entry.base_high = ((m_base >> 24) & 0xff)
...
struct GDT_Entry a_gdt_entries[6] = {
	...
	GDT_ENTRY(0,
			  0xfffff,
			  GDT_PRESENT | GDT_RING0 | GDT_CODE_SEGMENT | GDT_CODE_DATA_SEGMENT | GDT_READABLE,
			  GDT_GRANULARITY_4KIB | GDT_32BIT),

	// Data segment
	GDT_ENTRY(0,
			  0xfffff,
			  GDT_PRESENT | GDT_RING0 | GDT_DATA_SEGMENT | GDT_CODE_DATA_SEGMENT | GDT_WRITABLE,
			  GDT_GRANULARITY_4KIB | GDT_32BIT),
...
struct GDT_Description a_desc = {sizeof(a_gdt_entries) - 1, a_gdt_entries};
 ...
 */
void __attribute__((cdecl)) i386_gdt_load(struct GDT_Description *p_gdt,
										  uint8_t p_code_segment,
...
void i386_gdt_initialize()
{
	...
	GDT_ADDENTRY(a_gdt_entries[5],
				 (uint32_t)p_desc->address,
				 p_desc->size,
