...
/**
 * @brief Changes the present flag for the given ISR so that it is no longer checked when said interrupt is called,
 * usually to create a manual handler. Do not disable if you don't want to handle it manually, as the ISR call will
 * otherwise go to the BIOS, and may cause a reboot.
 * @param p_interrupt The interrupt to disable
 */
void i386_idt_disable_isr(int p_interrupt);

/**
 * @brief Registers the given ISR to call to the given function pointer (passed as void *) whenever the ISR is needed.
 * By default you should need to do this as all 256 ISRs are setup on launch of the kernel.
