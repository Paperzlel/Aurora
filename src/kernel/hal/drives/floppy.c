...
static void floppy_write_command(char command) {
    ...
    LOG_ERROR("Floppy timeout error, waited too long on command %hhx", command);
}
...
static void floppy_recalibrate(uint8_t drive_id) {
    ...
    LOG_ERROR("Controller failed to recalibrate.");
}
...
static bool floppy_drive_begin_rw(uint8_t drive_id, uint16_t lba, void *start, size_t size, bool is_write) {
    ...
    if (st0 != (0x20 | drive_id)) {
        LOG_ERROR("Seek command failed, had 0x%hhx, expected 0x%hhx.", st0, (0x20 | drive_id));
    }
    ...
    LOG_ERROR("Floppy disk timed out on read/write.");
    return false;
}
...
void floppy_initialize() {
    ...
    if (floppy_read_data() != 0x90) {
        LOG_ERROR("Floppy controller is not an 82077AA FDC. This drive type is unsupported.");
        return;
    }
...
void *floppy_read(uint8_t p_drive, uint16_t p_lba, void *p_to, size_t p_size) {
    ...
    LOG_ERROR("Failed to read information, see above errors.");
    return NULL;
}
...
bool floppy_write(uint8_t p_drive, uint16_t p_lba, void *p_from, size_t p_size) {
    ...
    LOG_ERROR("Failed to write information, see above errors.");
    return false;
}
