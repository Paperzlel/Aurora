...
/**
 * Controls both IRQs and regular interrupts. Handlers for each routine can be connected and disconnected here, as well
 * as the masking and unmasking of IRQs.
 */
...
/**
 * @brief Registers an interrupt handler for the given interrupt. Any times the interrupt is raised by code, the output
 * is redirected here instead of causing a kernel panic.
 ...
 * @return `true` if successful, and `false` if there already was an ISR there in the first place and we need to
 * unregister it.
 */
bool register_interrupt_handler(uint8_t p_interrupt_id, InterruptHandler p_handler);

/**
 * @brief Masks one of the 15 IRQs raised by the 8259 PIC. These are raised by the hardware, and interrupts outside of
...
/**
 * @brief Sends an End of Interrupt signal to the PIC, essentially telling it that the OS is ready for the next load of
 * interrupts to occur.
